...
a. a goal matches the head of a rule, in which case we should proceed with the body;
b. a goal is a conjunction (for instance, because it was produced in the previous step), of which each conjunct is derived separately;
c. a goal is among the assumptions.
...
a. if the resolvent contains a single atom, find a clause with that atom in the head and proceed with its body;
b. if the resolvent contains various atoms, start with the first and proceed with the rest;
c. if the resolvent is empty, we&rsquo;re done.

Some Prolog interpreters have problems if `clause` is called with the first argument instantiated to `true` or a conjunction, because `true` and &lsquo;`,`&rsquo; (comma) are built-in predicates. To avoid these problems, we should add the conditions `not A=true` and `not A=(X,Y)` to the first clause. A less declarative solution is to reorder the clauses and use cuts:
```Prolog
prove(true):-!.
prove((A,B)):-!,
    prove(A),
...
Note that this meta-interpreter is able to handle only &lsquo;pure&rsquo; Prolog programs, without system predicates like cut or `is`, since there are no explicit clauses for such predicates.

```{figure} ../images/part_i/image048.svg
---
name: 3.14
width: 60%
---
...
a. application of a different search strategy;
b. application of a different proof procedure;
c. enlargement of the set of clauses that can be handled;
d. extraction of additional information from the proof process.
