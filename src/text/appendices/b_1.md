...
a. replace implications by disjunction and negation;
b. push negations inside, so that each of them immediately precedes a literal;
c. move quantifiers to the front (the result is said to be in *prenex normal form*);
d. replace existentially quantified variables by Skolem functors;
e. rewrite into *conjunctive normal form*, i.e. a conjunction of disjunctions of literals;
f. rewrite each conjunct to a clause.
