...
likes(peter,S):-student_of(S,peter)
```
is seen as part of the definition of the `likes` predicate. Such a clause gives values for `X` and `Y` in which `likes(X,Y)` is true. In other words, it belongs to the *if* part of the definition: &lsquo;*X* likes *Y* if &hellip;&rsquo;. This definition can be *completed* by adding the *only-if* parts, resulting in a full definition: &lsquo;*X* likes *Y* if and only if &hellip;&rsquo;. Such a full definition is most easily expressed in Predicate Logic. For instance, the above clause could be completed to the following full definition:

$$
\forall \texttt{X} \forall \texttt{S} : \texttt{likes(X,S)} \leftrightarrow \texttt{X=peter} \land \texttt{student_of(S,peter)}
$$

In words: &lsquo;*X* likes *S* if and only if *X* is Peter, and *S* is a student of Peter&rsquo;, that is, Peter is the only one who likes people, and the people Peter likes are his students, and nobody else. We can translate this formula back to clausal form (see section 2.5), which yields a set of clauses
```Prolog
likes(peter,S):-student_of(S,peter).
X=peter:-likes(X,S).
