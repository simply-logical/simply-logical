...
a. a negated conjunction `not((A,B))` is explained by either explaining `not(A)` **or** `not(B)`;
b. if there are clauses for `A`, then `not(A)` is explained by constructing an explanation for `not(B)`, for **every** body `B`;
c. `not(A)` is explained if it is already part of the explanation;
d. otherwise, `not(A)` is explained by itself, if `A` is abducible and not explained;
e. `not(not(A))` is explained by explaining `A`.
...
```{swish} 8.3.2
---
source-text-start: 8.3.2-start
source-text-end: 8.3.2-end
---
```
...
```{figure} ../images/part_iii/image012.svg
---
width: 100%
name: 8.3
---
...
```{swish} 8.3.2_2
---
source-text-start: 8.3.2_2-start
...
```{swish} 8.3.2_3
---
source-text-start: 8.3.2_3-start
