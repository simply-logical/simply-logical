...
```{tip}
This example, which is used throughout {numref}`Chapters %s<ch:2>` and {numref}`%s<ch:3>`, assumes that bachelors are unmarried men. While this is till logically correct, it is perhaps time to look for a slightly less dated running example...
```
